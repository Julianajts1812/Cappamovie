const API_KEY = 'api_key=626ba72fc22b539f857560355e75274e';
  ...
  lastUrl = url;
    fetch(url).then(res => res.json()).then(data => {
        ...
        if(data.results.length !== 0){
            ...
            main.innerHTML= `<h1 class="no-results">Resultados não encontrados</h1>`
        }
  ...
  fetch(BASE_URL + '/movie/'+id+'/videos?'+API_KEY).then(res => res.json()).then(videoData => {
    ...
    if(videoData){
      ...
      if(videoData.results.length > 0){
        ...
        overlayContent.innerHTML = `<h1 class="no-results">Sem resultados</h1>`
      }
    }
  })
}
